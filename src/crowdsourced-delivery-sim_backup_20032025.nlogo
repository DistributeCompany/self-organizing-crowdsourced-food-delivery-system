...
restaurants-own[
  ...
  open-window-1?        ;; Open during nighttime and breakfast?
  open-window-2?        ;; Open during morning and afternoon?
  open-window-3?        ;; Open during early peak?
  open-window-4?        ;; Open during late peak and night?
  restaurant-type       ;; Type of restaurant (breakfast, day, evening, all-day)
]
...
globals [
  ...
  time-window-1-start   ;; Nighttime and breakfast start (e.g., 0)
  time-window-1-end     ;; Nighttime and breakfast end (e.g., 9)
  time-window-2-start   ;; Morning and afternoon start (e.g., 9)
  time-window-2-end     ;; Morning and afternoon end (e.g., 17.5)
  time-window-3-start   ;; Early peak start (e.g., 17.5)
  time-window-3-end     ;; Early peak end (e.g., 20)
  time-window-4-start   ;; Late peak and night start (e.g., 20)
  time-window-4-end     ;; Late peak and night end (e.g., 24)

  ;; Demand percentages for each time window (these should sum to 100%)
  time-window-1-percentage  ;; Nighttime and breakfast (00:00-09:00)
  time-window-2-percentage  ;; Morning and afternoon (09:00-17:30)
  time-window-3-percentage  ;; Early peak (17:30-20:00)
  time-window-4-percentage  ;; Late peak and night (20:00-00:00)

  ;; Restaurant type distributions
  breakfast-only-percentage    ;; Percentage of restaurants open only during breakfast
  day-only-percentage          ;; Percentage of restaurants open during day hours
  evening-only-percentage      ;; Percentage of restaurants open only during evening
  all-day-percentage           ;; Percentage of restaurants open all day

  ;; Order tracking variables
  open-orders-count            ;; Current number of open orders
  open-orders-history          ;; List to track open orders over time
  open-orders-by-window        ;; Table to track open orders by time window
  created-orders-by-window     ;; Table to track created orders by time window
  completed-orders-by-window   ;; Table to track completed orders by time window
]
...
to setup-time-based-demand
  ;; Set time window definitions (in hours, 24-hour format)
  set time-window-1-start 0
  set time-window-1-end 9
  set time-window-2-start 9
  set time-window-2-end 17.5
  set time-window-3-start 17.5
  set time-window-3-end 20
  set time-window-4-start 20
  set time-window-4-end 24

  ;; Set demand percentages (default values)
  set time-window-1-percentage 6
  set time-window-2-percentage 41
  set time-window-3-percentage 33
  set time-window-4-percentage 20

  ;; Set restaurant type distributions (default values)
  set breakfast-only-percentage 10
  set day-only-percentage 30
  set evening-only-percentage 20
  set all-day-percentage 40

  ;; Initialize order tracking
  set open-orders-count 0
  set open-orders-history []
  set open-orders-by-window table:make
  set created-orders-by-window table:make
  set completed-orders-by-window table:make

  ;; Initialize counters for each time window
  foreach (range 1 5) [ i ->
    table:put open-orders-by-window i 0
    table:put created-orders-by-window i 0
    table:put completed-orders-by-window i 0
  ]

  ;; Assign restaurant types and opening hours
  assign-restaurant-types
end

;; Assign restaurant types and opening hours
to assign-restaurant-types
  ask restaurants [
    ;; Determine restaurant type based on percentages
    let random-value random-float 100

    ifelse random-value < breakfast-only-percentage [
      ;; Breakfast-only restaurants (only open in window 1)
      set open-window-1? true
      set open-window-2? false
      set open-window-3? false
      set open-window-4? false
      set restaurant-type "breakfast"
    ][
      ifelse random-value < (breakfast-only-percentage + day-only-percentage) [
        ;; Day-only restaurants (open in windows 1 and 2)
        set open-window-1? false
        set open-window-2? true
        set open-window-3? false
        set open-window-4? false
        set restaurant-type "day"
      ][
        ifelse random-value < (breakfast-only-percentage + day-only-percentage + evening-only-percentage) [
          ;; Evening-only restaurants (open in windows 3 and 4)
          set open-window-1? false
          set open-window-2? false
          set open-window-3? true
          set open-window-4? true
          set restaurant-type "evening"
        ][
          ;; All-day restaurants (open in all windows)
          set open-window-1? true
          set open-window-2? true
          set open-window-3? true
          set open-window-4? true
          set restaurant-type "all-day"
        ]
      ]
    ]
  ]
...
;; Calculate current time of day in hours (0-24) based on ticks
;; Each tick is one second, so 3600 ticks = 1 hour
to-report current-time-of-day
  ;; Convert ticks to hours (3600 ticks = 1 hour)
  let hours-elapsed (ticks mod 86400) / 3600  ;; 86400 = seconds in a day
  report hours-elapsed
end

;; Determine current time window (1-4)
to-report current-time-window
  let time current-time-of-day

  ;; Check which time window we're in
  if time >= time-window-1-start and time < time-window-1-end [
    report 1
  ]
  if time >= time-window-2-start and time < time-window-2-end [
    report 2
  ]
  if time >= time-window-3-start and time < time-window-3-end [
    report 3
  ]
  if time >= time-window-4-start or time < time-window-1-start [
    report 4  ;; Handle cases around midnight
  ]
...
;; Calculate the demand multiplier for the current time window
to-report time-based-demand-multiplier
  let window current-time-window
  let day-length 24  ;; 24 hours in a day

  ;; Calculate the length of each window in hours
  let window-1-length time-window-1-end - time-window-1-start
  if window-1-length < 0 [ set window-1-length window-1-length + 24 ]

  let window-2-length time-window-2-end - time-window-2-start
  if window-2-length < 0 [ set window-2-length window-2-length + 24 ]

  let window-3-length time-window-3-end - time-window-3-start
  if window-3-length < 0 [ set window-3-length window-3-length + 24 ]

  let window-4-length time-window-4-end - time-window-4-start
  if window-4-length < 0 [ set window-4-length window-4-length + 24 ]

  ;; Calculate multipliers based on percentages
  ;; The multiplier should scale the demand so that over a full day,
  ;; we achieve the specified percentages of total demand
  let multiplier 1

  if window = 1 [
    ;; Convert percentage to a multiplier
    ;; If this time window has X% of total demand but comprises Y% of the day,
    ;; then we multiply by (X/Y)
    set multiplier (time-window-1-percentage / 100) / (window-1-length / day-length)
  ]
  if window = 2 [
    set multiplier (time-window-2-percentage / 100) / (window-2-length / day-length)
  ]
  if window = 3 [
    set multiplier (time-window-3-percentage / 100) / (window-3-length / day-length)
  ]
  if window = 4 [
    set multiplier (time-window-4-percentage / 100) / (window-4-length / day-length)
  ]

  report multiplier
end

;; Get open restaurants for the current time window
to-report open-restaurants
  let window current-time-window

  if window = 1 [
    report restaurants with [open-window-1?]
  ]
  if window = 2 [
    report restaurants with [open-window-2?]
  ]
  if window = 3 [
    report restaurants with [open-window-3?]
  ]
  if window = 4 [
    report restaurants with [open-window-4?]
  ]

  ;; Default fallback to all restaurants
  report restaurants
end
...
to create-job
  ;; Select only from restaurants that are open in the current time window
  let open-rests open-restaurants

  ;; Check if any restaurants are open
  ifelse any? open-rests [
    let temp-rest one-of open-rests
    ...
    set open-orders-count open-orders-count + 1

    ;; Track orders by time window
    let current-window current-time-window
    let current-count table:get created-orders-by-window current-window
    table:put created-orders-by-window current-window (current-count + 1)

    ;; Update open orders for this window
    let open-count table:get open-orders-by-window current-window
    table:put open-orders-by-window current-window (open-count + 1)

  ][
    ;; If no restaurants are open, do nothing
    if debug-demand-prediction [
      print (word "No restaurants open at current time: " current-time-of-day)
    ]
  ]
end

to process-job-creation
  ;; Get time-based multiplier for current demand
  let demand-multiplier time-based-demand-multiplier

  ;; Convert job-arrival-rate from "per hour" to "per second" (each tick is 1 second)
  let per-second-rate (job-arrival-rate * restaurant-clusters * restaurants-per-cluster) / 3600

  ;; Apply time window multiplier
  let actual-rate per-second-rate * demand-multiplier * 100

  ;; Create job based on adjusted rate
  if actual-rate > random-float 100 [
    create-job
  ]

  ;; Record open orders at regular intervals
  if ticks > 0 and ticks mod 300 = 0 [  ;; Log every 5 minutes
    set open-orders-history lput (list ticks open-orders-count) open-orders-history
  ]

  ;; Debug output if enabled
  if debug-demand-prediction and ticks mod 60 = 0 [  ;; Log every minute
    print (word "Time: " precision current-time-of-day 2 " hours, Window: " current-time-window
      ", Multiplier: " precision demand-multiplier 2
      ", Base rate: " precision per-second-rate 4 "/sec"
      ", Actual rate: " precision actual-rate 4 "%/tick")
  ]
end
...
;; Add this function to handle order completion (call when a job is completed)
to record-order-completion
  ;; Decrease open order count
  set open-orders-count max list 0 (open-orders-count - 1)

  ;; Track completion by time window
  let current-window current-time-window
  let completed-count table:get completed-orders-by-window current-window
  table:put completed-orders-by-window current-window (completed-count + 1)

  ;; Update open orders for this window
  let open-count table:get open-orders-by-window current-window
  table:put open-orders-by-window current-window max list 0 (open-count - 1)
end
...
to collect-run-data
  ...
  ;; Calculate open order metrics
  let avg-open-orders 0
  let max-open-orders 0

  if not empty? open-orders-history [
    set avg-open-orders mean map [ p -> last p ] open-orders-history
    set max-open-orders max map [ p -> last p ] open-orders-history
  ]
  ...
  ifelse table:has-key? results-data config-key [
    ...
    table:put run-results (word "run_" current-run) (list
      ...
      earnings-per-hour
      on-the-fly-jobs             ;; Jobs found while searching
      memory-jobs                 ;; Jobs found at restaurant
      job-type-ratio              ;; Ratio of on-the-fly to memory jobs
      avg-jobs-per-courier        ;; Average jobs per courier
      sd-jobs-per-courier         ;; Standard deviation of jobs per courier
      avg-open-orders             ;; Average number of open orders
      max-open-orders             ;; Maximum number of open orders
      table:get created-orders-by-window 1  ;; Orders created in window 1
      table:get created-orders-by-window 2  ;; Orders created in window 2
      table:get created-orders-by-window 3  ;; Orders created in window 3
      table:get created-orders-by-window 4  ;; Orders created in window 4
      table:get completed-orders-by-window 1 ;; Orders completed in window 1
      table:get completed-orders-by-window 2 ;; Orders completed in window 2
      table:get completed-orders-by-window 3 ;; Orders completed in window 3
      table:get completed-orders-by-window 4 ;; Orders completed in window 4
    )
...
to export-results
  ;; Create headers for CSV with additional metrics for open orders
  let csv-text "Configuration,DetailedConfiguration,Run,TotalDeliveries,AvgDeliveryTime,DeliveryTimeSD,AvgEarnings,EarningsSD,AvgDeliveriesPerCourier,DeliveriesPerCourierSD,CourierUtilization,WaitingPercentage,SearchingPercentage,EarningsPerHour,OnTheFlyJobs,MemoryJobs,JobTypeRatio,AvgJobsPerCourier,JobsPerCourierSD,AvgOpenOrders,MaxOpenOrders,CreatedWindow1,CreatedWindow2,CreatedWindow3,CreatedWindow4,CompletedWindow1,CompletedWindow2,CompletedWindow3,CompletedWindow4\n"
  ...
  foreach table:keys results-data [ config-key ->
    ...
    foreach table:keys config-results [ run-key ->
      ...
      set csv-text (word csv-text
        ...
        precision (item 16 run-data) 4 "\",\"" ;; AvgOpenOrders
        precision (item 17 run-data) 4 "\",\"" ;; MaxOpenOrders

        ;; Add created and completed orders by window
        item 18 run-data "\",\"" ;; CreatedWindow1
        item 19 run-data "\",\"" ;; CreatedWindow2
        item 20 run-data "\",\"" ;; CreatedWindow3
        item 21 run-data "\",\"" ;; CreatedWindow4
        item 22 run-data "\",\"" ;; CompletedWindow1
        item 23 run-data "\",\"" ;; CompletedWindow2
        item 24 run-data "\",\"" ;; CompletedWindow3
        item 25 run-data "\"" ;; CompletedWindow4
  ...
  carefully [
    ...
    ;; Also export the open orders time series data
    export-open-orders-data (word "open_orders_" date-and-time-report ".csv")
...
;; Function to export open orders data to CSV
to export-open-orders-data [file-path]
  if not empty? open-orders-history [
    file-open file-path

    ;; Write header
    file-print "tick,time_of_day,time_window,open_orders,created_window_1,created_window_2,created_window_3,created_window_4,completed_window_1,completed_window_2,completed_window_3,completed_window_4"

    ;; Write each data point
    foreach open-orders-history [ data-point ->
      let tick-value first data-point
      let order-count last data-point
      let time-of-day precision ((tick-value mod 86400) / 3600) 2

      ;; Determine time window for this data point
      let time-window 1
      if time-of-day >= time-window-2-start and time-of-day < time-window-2-end [
        set time-window 2
      ]
      if time-of-day >= time-window-3-start and time-of-day < time-window-3-end [
        set time-window 3
      ]
      if time-of-day >= time-window-4-start or time-of-day < time-window-1-start [
        set time-window 4
      ]
      ...
      let data-row (word
        ...
        time-window ","
        order-count ","
        table:get created-orders-by-window 1 ","
        table:get created-orders-by-window 2 ","
        table:get created-orders-by-window 3 ","
        table:get created-orders-by-window 4 ","
        table:get completed-orders-by-window 1 ","
        table:get completed-orders-by-window 2 ","
        table:get completed-orders-by-window 3 ","
        table:get completed-orders-by-window 4
      )
    ...
    file-close
    print (word "Open orders data exported to: " file-path)
  ]
end

to add-open-orders-to-results-data
  ;; Add open orders metrics to the results data
  ;; This assumes you're using the experiment framework from the original model

  ;; Add open orders count to results entry
  let config-key config-to-string current-config
  ...
  ifelse table:has-key? results-data config-key [
    ...
    let run-key (word "run_" current-run)
    if table:has-key? run-results run-key [
      let run-data table:get run-results run-key

      ;; Calculate average open orders
      let avg-open-orders 0
      if not empty? open-orders-history [
        set avg-open-orders mean map [ p -> last p ] open-orders-history
      ]

      ;; Calculate max open orders
      let max-open-orders 0
      if not empty? open-orders-history [
        set max-open-orders max map [ p -> last p ] open-orders-history
      ]

      ;; Add order metrics to run data
      let extended-run-data lput avg-open-orders run-data
      set extended-run-data lput max-open-orders extended-run-data
      set extended-run-data lput table:get created-orders-by-window 1 extended-run-data
      set extended-run-data lput table:get created-orders-by-window 2 extended-run-data
      set extended-run-data lput table:get created-orders-by-window 3 extended-run-data
      set extended-run-data lput table:get created-orders-by-window 4 extended-run-data

      ;; Update the results data
      table:put run-results run-key extended-run-data
      table:put results-data config-key run-results
    ]
  ]
  [
    print (word "Warning: Configuration key not found in results table: " config-key)
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
